...
# @param api_key String: The Datadog API key used for authentication.
#
# This class contains the Datadog installer installation mechanism for RHEL derivatives
#
...
class datadog_agent::redhat_installer (
  ...
  if ($rpm_repo_gpgcheck != undef) {
    ...
    if ($installer_repo_uri == undef) and ($agent_major_version > 5) {
      case $facts['os']['name'] {
