...
class datadog_agent::installer_params (
  ...
  exec { 'Prepare trace payload replacing template values':
    ...
    command => "
      sed -i \"s/TRACER_START_TIME_PLACEHOLDER/$(cat /tmp/puppet_start_time)/g\" /tmp/trace_payload.json
      sed -i \"s/TRACER_END_TIME_PLACEHOLDER/$(cat /tmp/puppet_stop_time)/g\" /tmp/trace_payload.json
      sed -i \"s/DURATION_PLACEHOLDER/$(echo $(expr $(cat /tmp/puppet_stop_time) - $(cat /tmp/puppet_start_time)))/g\" /tmp/trace_payload.json
      sed -i \"s/-4/0/g\" /tmp/trace_payload.json
      sed -i \"s/RUNTIME_ID_PLACEHOLDER/$(cat /tmp/datadog_trace_id)/g\" /tmp/trace_payload.json
    ...
    require => File['Trace payload pre-curl'],
  }
