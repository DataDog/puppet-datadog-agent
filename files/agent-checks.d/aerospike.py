...
# From https://github.com/DataDog/integrations-extras/tree/master/aerospike
...
class AerospikeCheck(AgentCheck):
        ...
        try:
            ...
            with closing(conn.makefile('r')) as fp:
                ...
                for ns in namespaces:
                    ...
                    # self.log.debug("AF namespace_metrics=%s", str(namespace_metrics))
    ...
    def _process_data(self, instance, fp, metric_type, required_keys=[], tags={}, delim=';'):
        ...
        for key, value in required_data.items():
            # self.log.info("AF  key=%s  value=%s", key, str(value))
    ...
    def _send(self, metric_type, key, val, tags={}):
        ...
        if datatype == 'gauge':
            # self.log.info("AF _send() gauge key=%s  value=%s", key, str(val))
            self.gauge(self._make_key(metric_type, key), val, tags=tags)
        elif datatype == 'rate':
            # self.log.info("AF _send() rate key=%s  value=%s", key, str(val))
