# @summary Contains the parameters for the Datadog module
#
#
class datadog_agent::params {
  ...
  case $facts['os']['name'] {
    ...
    'Windows': {
      # Not a real path, but integrations use it to ensure => absent so it needs to be a valid path
      $legacy_conf_dir            = 'C:/ProgramData/Datadog/agent5'
      $conf_dir                   = 'C:/ProgramData/Datadog/conf.d'
      $dd_user                    = 'ddagentuser'
      # Administrators group, passed as SID so it works on localized Windows versions
      ...
      $package_name               = 'Datadog Agent'
      # On Windows, the Administrators group needs to maintain access,
      # otherwise puppet itself won't be able to access the file. Reported
      # as bug in: https://tickets.puppetlabs.com/browse/PA-2877
